...
#	endif
	...
	TestSpec{{}, "prctl", {
...
#endif
			R"(prctl\(op=PR_GET_THP_DISABLE\) = (DISABLED_EXCEPT_ADVISED|UNSPECIFIED) \(config\))",
			...
			R"(prctl\(op=PR_GET_TID_ADDRESS, addrp={addr} → \[{addr}\]\) = (22|0))",
			R"(prctl\(op=PR_SET_TIMERSLACK, slack={decimal}\) = 0 \(success\))",
...
#	endif
	...
	TestSpec{"socket", "socket,bind,connect,listen,accept,accept4,shutdown,getpeername,getsockname,recvfrom,sendto,sendmsg,recvmsg,sendmmsg,recvmmsg", {
		...
		R"(bind\(sockfd=3, addr=\{family=AF_UNIX, path=\"@testsocket\"\}, addrlen=14\) = 0 \(success\))",
		R"(bind\(sockfd=3, addr=\{family=AF_INET6, port=1234, flowinfo=4321, addr=\"::1\", scope_id=0\}, addrlen=28\) = 0 \(success\))",
		R"(bind\(sockfd=3, addr=\{family=AF_INET, port=1234, addr=\"127.0.0.1\"\}, addrlen=16\) = 0 \(success\))",
		R"(connect\(sockfd=4, addr=\{family=AF_UNIX, path="@testsocket"\}, addrlen=14\) = 0 \(success\))",
		...
		R"(getsockname\(sockfd={fd}, addr=\{family=AF_UNIX, path="@testsocket"\}, addrlen=110 → 14\) = 0 \(success\))",
		...
		R"(sendmmsg\(sockfd={fd}, msgvec=\[\{msg_hdr=\{msg_name=\{family=AF_UNIX, path="@sendsocket"\}, msg_namelen=14, msg_iov=\[\{iov_base={addr} → \["1: test message 1"\], iov_len=17\}\], msg_iovlen=1, msg_control=NULL, msg_controllen=0, msg_flags=0x0 \(\)\}, msg_len=17\}, \{msg_hdr=\{msg_name=NULL, msg_namelen=0, msg_iov=\[\{iov_base={addr} → \["2: test message 2"\], iov_len=17\}\], msg_iovlen=1, msg_control=NULL, msg_controllen=0, msg_flags=0x0 (\\)\}, msg_len=17\}\], num_msgs=2, flags=0x4000 \(MSG_NOSIGNAL\)\) = 2 \(num_updated\))",
		R"(recvmmsg\(sockfd=4, msgvec=\[\{msg_hdr=\{msg_name=\{family=AF_UNIX, path="@testsocket"\}, msg_namelen=110 → 14, msg_iov=\[\{iov_base={addr} → ["1: test message 1"], iov_len=1024}], msg_iovlen=1, msg_control=\[\{cmsg_len=12, cmsg_level=SOL_SOCKET, cmsg_type=SCM_CREDENTIALS, cmsg_data=\{pid={pid}, uid={uid}, gid={gid}}}], msg_controllen=1024 → 32, msg_flags=0x0 \(\)\}, msg_len=17\}, \{msg_hdr=\{msg_name=NULL, msg_namelen=0, msg_iov=\[\{iov_base={addr} → \["2: test message 2"\], iov_len=1024\}\], msg_iovlen=1, msg_control=NULL, msg_controllen=0, msg_flags=0x8 \(MSG_CTRUNC\)\}, msg_len=17\}\], num_msgs=2, flags=0x4000 \(MSG_NOSIGNAL\), timeout=\{10s, 512ns\} → left: \{{decimal}s, {decimal}ns\}\) = 2 \(num_received\))",
		R"(socket\(domain=AF_NETLINK, type=0x2 \(SOCK_DGRAM\), prot=NETLINK_USERSOCK\) = {fd} \(fd\))",
		R"(getsockname\(sockfd={fd}, addr=\{family=AF_NETLINK, nl_pad=0, nl_pid={decimal}, nl_groups=0\}, addrlen=12 → 12\) = 0 \(success\))",
		R"(socket\(domain=AF_PACKET, type=0x2 \(SOCK_DGRAM\), prot=ETH_P_REALTEK\) = 1 \(EPERM\) \(errno\))",
	}},
	...
	TestSpec{"socket", "socketcall", {
		...
		R"(socketcall\(call=SYS_BIND, args=\{sockfd={fd}, sockaddr=\{family=AF_INET6, port=1234, flowinfo=4321, addr="::1", scope_id=0\}, addrlen=28\}\) = 0 \(success\))",
		R"(socketcall\(call=SYS_CONNECT, args=\{sockfd={fd}, sockaddr=\{family=AF_INET6, port=1234, flowinfo=4321, addr="::1", scope_id=0\}, addrlen=28\}\) = 0 \(success\))",
		R"(socketcall\(call=SYS_LISTEN, args=\{sockfd={fd}, backlog=15\}\) = 95 \(EOPNOTSUPP\) \(errno\))",
		R"(socketcall\(call=SYS_ACCEPT, args=\{sockfd={fd}, addr=\{family=AF_UNIX, path="<unnamed>"\}, addrlen=110 → 2\}\) = 4 \(fd\))",
		R"(socketcall\(call=SYS_ACCEPT4, args=\{sockfd={fd}, addr=\{family=AF_UNIX, path="<unnamed>"\}, addrlen=110 → 2, flags=0x800 \(SOCK_NONBLOCK\)\}\) = 4 \(fd\))",
		R"(socketcall\(call=SYS_SHUTDOWN, args=\{sockfd={fd}, how=SHUT_WR\}\) = 0 \(success\))",
		R"(socketcall\(call=SYS_GETPEERNAME, args=\{sockfd={fd}, addr=\{family=AF_INET6, port=1234, flowinfo=0, addr="::1", scope_id=0\}, addrlen=28 → 28\}\) = 0 \(success\))",
		R"(socketcall\(call=SYS_GETSOCKNAME, args=\{sockfd={fd}, addr=\{family=AF_INET6, port=1234, flowinfo=0, addr="::1", scope_id=0\}, addrlen=28 → 28\}\) = 0 \(success\))",
		R"(socketcall\(call=SYS_SEND, args=\{sockfd={fd}, buf="test message", count=12, flags=0x4000 \(MSG_NOSIGNAL\)\}\) = 12 \(bytes\))",
		...
		R"(socketcall\(call=SYS_SENDMMSG, args=\{sockfd={fd}, msgvec=\[\{msg_hdr=\{msg_name=\{family=AF_UNIX, path="@sendsocket"\}, msg_namelen=14, msg_iov=\[\{iov_base={addr} → ["1: test message 1"], iov_len=17\}]\, msg_iovlen=1, msg_control=NULL, msg_controllen=0, msg_flags=0x0 \(\)\}, msg_len=17\}, \{msg_hdr=\{msg_name=NULL, msg_namelen=0, msg_iov=\[\{iov_base={addr} → \["2: test message 2"\], iov_len=17\}\], msg_iovlen=1, msg_control=NULL, msg_controllen=0, msg_flags=0x0 \(\)\}, msg_len=17\}\], num_msgs=2, flags=0x4000 \(MSG_NOSIGNAL\)\}\) = 2 \(num_updated\))",
		R"(socketcall\(call=SYS_RECVMMSG, args=\{sockfd={fd}, msgvec=\[\{msg_hdr=\{msg_name=\{family=AF_UNIX, path="@testsocket"\}, msg_namelen=110 → 14, msg_iov=\[\{iov_base={addr} → \["1: test message 1"\], iov_len=1024\}\], msg_iovlen=1, msg_control=\[\{cmsg_len=12, cmsg_level=SOL_SOCKET, cmsg_type=SCM_CREDENTIALS, cmsg_data=\{pid={pid}, uid={uid}, gid={gid}\}\}\]\, msg_controllen=1024 → 24, msg_flags=24\}, msg_len=17\}, \{msg_hdr=\{msg_name=NULL, msg_namelen=0, msg_iov=\[\{iov_base={addr} → \["2: test message 2"\], iov_len=1024\}\], msg_iovlen=1, msg_control=NULL, msg_controllen=0, msg_flags=0\}, msg_len=17\}\], num_msgs=2, flags=0x4000 \(MSG_NOSIGNAL\), timeout=\{10s, 0ns\} → left: \{{decimal}s, {decimal}ns\}\}\) = 2 \(num_received\))",
	}},
